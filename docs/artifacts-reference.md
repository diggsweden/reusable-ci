...
- **Behavior:**
  ...
  - **Gradle / Gradle Android** — gates publish targets rather than the
    ...
    Play listing) and admits `github-packages`.
...
- **Why not the bare `publish` task:** `publish` pushes every publication
  ...
  destinations would publish to Central from inside the GitHub Packages
...
- **Applies to:** Maven libraries, NPM packages, Gradle artefacts, and
  ...
  are allowed — publishing an application jar to GitHub Packages is a
...
- **Gradle note:** routes to `publish-gradle.yml`, which publishes from
  ...
  [publishing.md](publishing.md#maven-central-and-github-packages-gradle).
