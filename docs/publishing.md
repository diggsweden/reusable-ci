...
| GitHub Packages (Gradle)  | Gradle artefacts (incl. Android libraries) | `GITHUB_TOKEN` (automatic) |
...
## Maven Central and GitHub Packages (Gradle)
...
Central from inside the GitHub Packages job.
...
|---|---|---|
| `githubActor` / `githubToken` | ambient job context | github-packages |
...
artifacts:
  - name: my-package
    ...
    publish-to:
      - github-packages  # Publish to GitHub Packages
    config:
      node-version: 24
```
...
- **SLSA Provenance** - Level 3 attestations for supported GHCR container publishes when enabled
