...
`npm pack`, etc.). Container-first ecosystems have nothing to build in the
build stage — the actual compile happens inside the Containerfile in the
publish stage. So their lockfile-derived "build" SBOM (`sbom-cargo.yml`)
runs in `release-publish-stage` as a sibling of `build-containers`,
shipping with the container it documents. For pure-cargo projects
...
| SBOM — build layer | ✅ | `sbom-cargo.yml` (cargo-cyclonedx) | Lockfile-derived (reads `Cargo.lock`); runs in publish stage as a sibling of the container build, since cargo is container-first and "build" doesn't happen until the container compiles. |
