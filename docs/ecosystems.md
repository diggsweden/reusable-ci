...
(`git log -1 --format=%ct HEAD`) and passes it via the BuildKit step env on
both the main build and the optional `extract.binary` build. BuildKit ≥ 0.11
uses this to clamp the **image-config `created` field** and **every layer-tar
...
| Publish — GitHub Packages | ✅ | `publish-gradle.yml` | Applications may publish here too; only Central requires `build-type: library`. |
...
| Publish — GitHub Packages (AAR) | ✅ | `publish-gradle.yml` | Android **libraries** only (`build-type: library`). |
...
| Build SBOM | ✅ `build-go.yml` runs `reusable-ci build go sbom` (cyclonedx-gomod) | ✅ `sbom-go.yml` runs the same, in the publish stage |
