...
described by the provider boundary in
...
- **Composes the quality stage** by including the `nanolinter` component, which
  ...
  (the flavour image bakes the tools, so no `just`/justfile is needed). Add further
...
the merge request, go red. There is no separate "status" aggregation job as on
...
job's result via `toJson(needs)`. GitLab has no equivalent, because a job cannot see its
