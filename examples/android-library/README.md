...
Maven Central and/or the forge's own package registry.

## Use Case

Android libraries — as opposed to applications — that need to:

- Publish an AAR to Maven Central and/or the forge's package registry
...
|---|---|---|
| Publish targets | `maven-central`, `forge-packages` | `google-play` |
...
Forge packages needs no secrets — it uses the auto-provided
`GITHUB_TOKEN`.
