#!/usr/bin/env bash
# SPDX-FileCopyrightText: 2025 Digg - Agency for Digital Government
# SPDX-License-Identifier: CC0-1.0

# Create SBOM ZIP archive containing all 3 layers, optionally sign with GPG
# Usage: create-sbom-zip.sh <project-name> <version>
# Optional env vars: SIGN_ARTIFACTS=true GPG_KEY_ID=<key-id>

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
source "$SCRIPT_DIR/../ci/output.sh"

main() {
  local PROJECT_NAME="${1:-$(basename "$(git config --get remote.origin.url)" .git)}"
  local VERSION="${2:-unknown}"
  VERSION="${VERSION#v}" # Remove 'v' prefix if present

  # Count SBOMs to determine if we need to create a ZIP
  local SBOM_COUNT
  SBOM_COUNT=$(find . -maxdepth 1 -name '*-sbom.*.json' 2>/dev/null | wc -l)
  if [[ -d "./sbom-artifacts" ]]; then
    SBOM_COUNT=$((SBOM_COUNT + $(find ./sbom-artifacts -name '*-analyzed-container-sbom.*.json' 2>/dev/null | wc -l)))
  fi

  if [[ "$SBOM_COUNT" -eq 0 ]]; then
    printf "No SBOMs found, skipping ZIP creation\n"
    exit 0
  fi

  printf "Creating SBOM zip archive with all 3 layers\n"
  local SBOM_ZIP
  SBOM_ZIP=$(ci_sbom_zip_name "$PROJECT_NAME" "$VERSION")

  # Add build- and analyzed-* layer SBOMs to ZIP
  # Matches any layer generated by generate-sboms.sh (build, analyzed-jar, analyzed-tararchive, analyzed-binary, analyzed-wheel, etc.)
  for sbom in *-sbom.spdx.json *-sbom.cyclonedx.json; do
    if [[ -f "$sbom" ]]; then
      zip "$SBOM_ZIP" "$sbom"
      printf "  Added: %s\n" "$sbom"
    fi
  done

  # Add analyzed-container SBOMs to zip if they exist
  if [[ -d "./sbom-artifacts" ]]; then
    for sbom in ./sbom-artifacts/*-analyzed-container-sbom.*.json; do
      if [[ -f "$sbom" ]]; then
        zip -j "$SBOM_ZIP" "$sbom"
        printf "  Added: %s\n" "$(basename "$sbom")"
      fi
    done
  fi

  printf "SBOM ZIP contents:\n"
  unzip -l "$SBOM_ZIP"

  if [[ "${SIGN_ARTIFACTS:-false}" == "true" ]] && [[ -n "${GPG_KEY_ID:-}" ]]; then
    ci_gpg_sign "$GPG_KEY_ID" "$SBOM_ZIP"
    printf "Signed SBOM ZIP: %s.asc\n" "$SBOM_ZIP"
  fi

  printf "Created SBOM ZIP: %s\n" "$SBOM_ZIP"
}

main "$@"
