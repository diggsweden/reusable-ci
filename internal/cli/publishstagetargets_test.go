...
//
// This closes a silent failure mode. `report status stage` maps job id →
...
func TestPublishStageTargets_HaveMatchingJobs(t *testing.T) {
	...
	for _, target := range publishTargetKeys(t) {
		...
		if _, exists := wf.Jobs[jobID]; !exists {
			t.Errorf("publish-stage target %q has no job in release-publish-stage.yml (tried %q and %q); `report status stage` would silently report it as not run",
				target, target, jobID)
