...
func dryRunFlag() cli.Flag {
	// -n is the clig.dev-standard short form for --dry-run (rsync, git add).
	return &cli.BoolFlag{Name: "dry-run", Aliases: []string{"n"}, Usage: "preview registry mutations (copies/deletes) without performing them"}
}
   ...
   reusable-ci container ledger add --kind alpine \
     ...
     --sbom dist/image-sbom-alpine.cyclonedx.json --tag v1.2.3`,
		Flags: []cli.Flag{
			...
			&cli.StringFlag{Name: "digest", Usage: "image digest (sha256:<64 hex>)"},
...
func ledgerMergeCmd() *cli.Command {
	return &cli.Command{
		...
		Action: func(_ context.Context, cmd *cli.Command) error {
			...
			// forge-aware annotation (::warning:: on GHA/Forgejo) so it is
			// visible rather than a silently green run.
