...
func checkoutCmd() *cli.Command {
	return &cli.Command{
		...
		Description: "Git-based checkout for jobs where the JavaScript actions/checkout cannot run " +
			"(node-less containers, sha256 repos). Resolves the object format from the forge, " +
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{Name: "ref", Sources: cienv.Commit(), Usage: "commit SHA, refs/tags/…, refs/heads/…, or a bare tag/branch name (default: triggering commit)"},
...
// actions/checkout path:) wins and is resolved relative to $GITHUB_WORKSPACE
// (or cwd when unset), kept absolute so the app's safe.directory guard stays
// correct. Otherwise an explicit --workspace, else the current directory.
