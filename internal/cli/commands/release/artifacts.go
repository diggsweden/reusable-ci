...
func buildGPGSigner(cmd *cli.Command, method domainrelease.SignMethod) (apprelease.Signer, domainrelease.SignMethod, error) {
	...
	if privateKey == "" {
		return nil, "", fmt.Errorf("sign: the GPG private key is required: pass --private-key-file <path> (\"-\" for stdin) or set $GPG_PRIVATE_KEY: %w", errs.ErrUsage)
	}
...
func signCmd() *cli.Command {
	flags := append([]cli.Flag{
		...
		&cli.StringFlag{Name: "assembly", Sources: cli.EnvVars("RELEASE_ASSEMBLY"), Usage: "release assembly manifest to sign exactly"},
...
func downloadArtifactsCmd() *cli.Command {
	return &cli.Command{
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{Name: "repository", Sources: cienv.Repository(), Usage: "\"owner/repo\" the run belongs to"}, //nolint:goconst // test fixture / generic identifier — extracting would explode setup boilerplate.
		},
		Action: func(ctx context.Context, cmd *cli.Command) error {
...
func checksumsCmd() *cli.Command {
	return &cli.Command{
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{Name: "assembly", Sources: cli.EnvVars("RELEASE_ASSEMBLY"), Usage: "release assembly manifest to checksum exactly"},
...
func sbomZipCmd() *cli.Command {
	flags := append([]cli.Flag{
		...
		&cli.StringFlag{Name: "assembly", Sources: cli.EnvVars("RELEASE_ASSEMBLY"), Usage: "release assembly manifest whose SBOM inputs should be bundled"},
		debugAllowSwapFlag(),
	}, signMethodFlags()...)
