...
func TestGenerateCmd_InvalidProjectTypeFailsFast(t *testing.T) {
	...
	err := cmd.Run(context.Background(), []string{"sbom", "generate", "all", "--project-type", "unknown"}) //nolint:goconst // test fixture / generic identifier — extracting would explode setup boilerplate.
...
func TestGenerateContainerCmd_RequiresRefNameFromEnv(t *testing.T) {
	...
	err := cmd.Run(context.Background(), []string{"sbom", "generate", "container"}) //nolint:goconst // test fixture / generic identifier — extracting would explode setup boilerplate.
