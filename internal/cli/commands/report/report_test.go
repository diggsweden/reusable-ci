...
import (
	...
	reportcmd "github.com/diggsweden/reusable-ci/v3/internal/cli/commands/report"
...
func TestStageResultCmd_WritesOutputsAndManifest(t *testing.T) {
	...
	cmd := reportcmd.New()
	if err := cmd.Run(context.Background(), []string{"report", "stage-result", "--result", "maven=success", "--extra", "project_type=maven"}); err != nil {
		t.Fatal(err)
	}
