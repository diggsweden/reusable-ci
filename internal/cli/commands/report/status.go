...
func statusStageCmd() *cli.Command {
	return &cli.Command{
		Name:  "stage",
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{Name: "needs-json", Sources: cli.EnvVars("NEEDS_JSON"), Usage: "GHA toJson(needs) payload; mutually exclusive with --result"},
			&cli.StringSliceFlag{Name: "result", Usage: "target=result pair; repeat for every job result (legacy)"},
		...
		Action: func(ctx context.Context, cmd *cli.Command) error {
			...
			return deps.FromCmd(ctx, cmd, func(d *deps.Deps) error {
				_, err := appsummary.StageResult(ctx, d.OutputSink, d.ManifestSink, appsummary.StageResultInput{
					StagePlanJSON: cmd.String("stage-plan-json"),
					NeedsJSON:     cmd.String("needs-json"),
					Results:       results,
					Extras:        extras,
...
func statusPrerequisitesCmd() *cli.Command {
	return &cli.Command{
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{Name: "publish-to", Sources: cli.EnvVars("PUBLISH_TO"), Usage: "comma-separated publish targets in the plan (maven-central/npm/…)"},
