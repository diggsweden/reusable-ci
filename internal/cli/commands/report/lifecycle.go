...
func prCmd() *cli.Command {
   ...
   reusable-ci report pr --project-type maven --source-branch feature/x --actor octocat`,
		Flags: []cli.Flag{
			...
			&cli.StringFlag{Name: "source-branch", Sources: cli.EnvVars("CI_BRANCH"), Usage: "PR source branch"},
			&cli.StringFlag{Name: "commit", Sources: cienv.Commit(), Usage: "head commit SHA of the PR"},
			&cli.StringFlag{Name: "actor", Sources: cienv.Actor(), Usage: "user who opened/updated the PR"},
			&cli.StringFlag{Name: "run-url", Sources: cienv.RunURL(), Usage: "URL of the CI run linked from the summary"}, //nolint:goconst // test fixture / generic identifier — extracting would explode setup boilerplate.
		...
		Action: func(ctx context.Context, cmd *cli.Command) error {
			...
			return deps.FromCmd(ctx, cmd, func(d *deps.Deps) error {
				return appsummary.PRSummary(ctx, d.SummarySink, appsummary.PRSummaryInput{
					...
					Branch:                 cmd.String("source-branch"),
...
func releaseCmd() *cli.Command {
   ...
   reusable-ci report release --release-version v1.2.3 --release-branch main`,
		Flags: []cli.Flag{
			...
			&cli.StringFlag{Name: "run-url", Sources: cienv.RunURL(), Usage: "URL of the CI run linked from the summary"},
			...
			&cli.StringFlag{Name: "promote-snapshot-result", Sources: cli.EnvVars("PROMOTE_DEV_RESULT"), Usage: "outcome of the image promote-dev job (success/failure/skipped)"},
			&cli.StringFlag{Name: "promote-staging-result", Sources: cli.EnvVars("PROMOTE_STAGING_RESULT"), Usage: "outcome of the image promote-staging job (success/failure/skipped)"},
			&cli.StringFlag{Name: "promote-release-result", Sources: cli.EnvVars("PROMOTE_RELEASE_RESULT"), Usage: "outcome of the image promote-release job (success/failure/skipped)"},
			...
			&cli.StringFlag{Name: "server-url", Sources: cienv.ServerURL(), Usage: "CI server base URL (used to build release/run links)"},
			&cli.StringFlag{Name: "repository", Sources: cienv.Repository(), Usage: "\"owner/repo\" used in the release link"}, //nolint:goconst // test fixture / generic identifier — extracting would explode setup boilerplate.
		},
		Action: func(ctx context.Context, cmd *cli.Command) error {
			return deps.FromCmd(ctx, cmd, func(d *deps.Deps) error { //nolint:varnamelen // idiomatic short name (testing/http/io conventions).
				return appsummary.ReleaseSummary(ctx, d.SummarySink, appsummary.ReleaseSummaryInput{
					...
					PromoteDevResult:     cmd.String("promote-snapshot-result"),
					PromoteStagingResult: cmd.String("promote-staging-result"),
					PromoteReleaseResult: cmd.String("promote-release-result"),
...
func snapshotReleaseCmd() *cli.Command {
	return &cli.Command{
		...
		Usage: "append the snapshot-release step-summary (job table, npm install snippet, and links)",
		Description: `EXAMPLE:
   reusable-ci report snapshot-release --project-type npm --release-sha abc1234`,
		Flags: []cli.Flag{
			...
			&cli.StringFlag{Name: "release-ref", Sources: cli.EnvVars("RELEASE_REF"), Usage: "git ref the snapshot-release was cut from"},
			&cli.StringFlag{Name: "release-sha", Sources: cli.EnvVars("RELEASE_SHA"), Usage: "commit SHA the snapshot-release was cut from"},
			&cli.StringFlag{Name: "release-actor", Sources: cli.EnvVars("RELEASE_ACTOR"), Usage: "user who triggered the snapshot-release"},
			&cli.StringFlag{Name: "release-repository", Sources: cli.EnvVars("RELEASE_REPOSITORY"), Usage: "\"owner/repo\" the snapshot-release was published from"},
			&cli.StringFlag{Name: "run-url", Sources: cienv.RunURL(), Usage: "URL of the CI run linked from the summary"},
			...
			&cli.StringFlag{Name: "snapshot-artifacts-json", Sources: cli.EnvVars("SNAPSHOT_ARTIFACTS_JSON"), Usage: "inline JSON listing the snapshot artifacts shown in the summary"},
			&cli.StringFlag{Name: "server-url", Sources: cienv.ServerURL(), Usage: "CI server base URL (used to build artifact/run links)"},
		},
		Action: func(ctx context.Context, cmd *cli.Command) error {
			return deps.FromCmd(ctx, cmd, func(d *deps.Deps) error { //nolint:varnamelen // idiomatic short name (testing/http/io conventions).
				return appsummary.SnapshotReleaseSummary(ctx, d.SummarySink, os.Stderr, appsummary.SnapshotReleaseSummaryInput{
