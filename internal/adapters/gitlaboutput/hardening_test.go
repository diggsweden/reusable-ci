...
// The GitLab dotenv output path is supplied by the job, and the sink appends
// every published value to it — versions, digests, tags, release SHAs — in a
// format a later job reads back as variables.
//
// Following a symlink there writes all of that somewhere the job never
// nominated, under a name an earlier step or a checked-out repository could
// have created first. Worse than the GitHub case in one respect: a dotenv file
// is SOURCED by the next job, so a redirected write lands in a file whose
// contents become environment variables. The sink shares cliio's hardened open
// now, which refuses when the final component is a link.
...
func TestSink_StillAppendsToARealOutputFile(t *testing.T) {
	...
	// dotenv keys are uppercased, because a later job sources them as shell
	// variables.
