...
//
// Forgejo runs on a GitHub-Actions-compatible runner but speaks the
// Gitea /api/v1 REST surface, which Forgejo keeps API-compatible. We
// talk to it through the official Gitea Go SDK (code.gitea.io/sdk/gitea)
// — the same "use the forge's SDK" shape the github adapter uses with
// go-github, rather than the hand-rolled REST in the gitlab adapter.
//
