...
//
// AddPathspecsStrict below deliberately stays batched: its pathspecs are
// all required, so git's abort-the-whole-invocation behaviour is the
// wanted one there — a missing changelog must fail, not be skipped.
