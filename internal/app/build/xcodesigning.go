...
// keychain, imports the cert, and installs the profile. Mirrors
// scripts/apple/setup-code-signing.sh end-to-end.
//
// Pure darwin operation — the linux unit tests use a fake
// XcodeSecurityOps that records the security-CLI invocations.
func XcodeSetupCodeSigning(ctx context.Context, sec XcodeSecurityOps, stdout io.Writer, in XcodeSetupCodeSigningInput) error {
	if in.CertBase64 == "" {
	...
	fmt.Fprintln(stdout, "✓ Code signing configured successfully")
	return nil
}
