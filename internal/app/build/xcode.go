...
func XcodeXCConfig(ctx context.Context, sink ci.OutputSink, annot output.Annotator, in XcodeXCConfigInput) error {
	path, err := resolveXcodeXCConfig(in)
	if err != nil {
		return err
	}
...
// resolveXcodeXCConfig decodes the xcconfig secret to a tempfile and returns
// its path, or "" when no secret is set. Shared by XcodeXCConfig (emits the
// path) and XcodeReleaseBuild (threads it to the archive in-process).
func resolveXcodeXCConfig(in XcodeXCConfigInput) (string, error) {
