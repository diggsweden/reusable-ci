...
// resolveNPMMetadata reads package.json and validates that name + version are
// present. Shared by NPMMetadata (which emits them) and NPMReleaseBuild (which
// threads them through the build). Scope validation is the caller's, since it
// differs by surface (annotated error vs plain).
func resolveNPMMetadata(in NPMMetadataInput) (npmPackageJSON, error) {
	meta, err := readNPMPackageJSON(defaultDir(in.Dir))
	if err != nil {
...
// npmScopeMismatch reports whether name violates a required package scope.
func npmScopeMismatch(name, scope string) bool {
	return scope != "" && !strings.HasPrefix(name, strings.TrimRight(scope, "/")+"/")
}
...
func NPMMetadata(ctx context.Context, sink ci.OutputSink, w io.Writer, annot output.Annotator, in NPMMetadataInput) error { //nolint:varnamelen // idiomatic short name (testing/http/io conventions).
	meta, err := resolveNPMMetadata(in)
	if err != nil {
		return err
	}
