...
func TestValidateContainerfile_ExactPathExists(t *testing.T) {
	...
	if err := appcontainer.ValidateContainerfile(context.Background(), sink, &stdout, appcontainer.ValidateContainerfileInput{Path: path}); err != nil {
...
func TestValidateContainerfile_SingleGlobMatch(t *testing.T) {
	...
	sink := fakeoutputsink.New(t)
	var stdout bytes.Buffer
	if err := appcontainer.ValidateContainerfile(context.Background(), sink, &stdout, appcontainer.ValidateContainerfileInput{
