...
// RegistryLogoutInput drives RegistryLogout.
type RegistryLogoutInput struct {
	Registry string
	// AuthFile overrides where credentials are read/written. Empty resolves the
	// tool-neutral default (see authFilePath) — the same precedence as login.
	AuthFile string
}

// RegistryLogout removes registry's credential from the shared OCI auth config —
// the inverse of RegistryLogin, for long-lived (self-hosted) runners where the
