...
func generateBuildLayer(
	...
	_, _ = fmt.Fprintln(w, "📦 Assembling Build layer (harvest-or-generate)...")
	...
	files := maybeGenerateBuildBOM(ctx, ws, gen, projectType, name, root, w, stderr)
...
func maybeGenerateBuildBOM(ctx context.Context, ws workspace, gen BuildSBOMGenerator, projectType projecttype.Type, name, root string, w, stderr io.Writer) []string { //nolint:varnamelen // idiomatic short names (w) — testing/http/io conventions.
	files := walkAllFiles(ws, root)

	if gen == nil || !noBuildBOM(files) {
		return files
	}

	if genErr := gen.GenerateBuildSBOM(ctx, projectType, ".", name, stderr); genErr != nil {
		if !errors.Is(genErr, errs.ErrUnsupported) {
			_, _ = fmt.Fprintf(w, "   %s generate build BOM: %v\n", clicolor.Cross(w), genErr)
		}

		return files
	}

	return walkAllFiles(ws, root) // re-walk to pick up the generated bom.json
}
...
func emitBuildBOMSource(ws workspace, root, src, name, version, sha, stack, hint string, w io.Writer) { //nolint:varnamelen // idiomatic short name (testing/http/io conventions).
	...
	_, _ = fmt.Fprintf(w, "   %s %s (harvested from %s)\n", clicolor.Check(w), out, srcPath)
}
...
func generateArtifactLayer(
	...
	_, _ = fmt.Fprintln(w, "📦 Assembling Artifact layer (syft scan)...")
	_, _ = fmt.Fprintf(w, "   Project type: %s\n", projectType)
...
func generateContainerLayer(
	...
	_, _ = fmt.Fprintln(w, "📦 Assembling Container layer (syft scan)...")
