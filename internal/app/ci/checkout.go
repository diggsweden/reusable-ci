...
type CheckoutGit interface {
	...
	Fetch(ctx context.Context, remoteURL string, refspecs []string, token string) error
...
func fetchExtras(ctx context.Context, git CheckoutGit, remoteURL string, in CheckoutInput) error {
	if in.FetchBase != "" {
		_ = git.Fetch(ctx, remoteURL,
			[]string{"+refs/heads/" + in.FetchBase + ":refs/heads/" + in.FetchBase}, in.Token)
	}
...
func fetchAndCheckout(ctx context.Context, git CheckoutGit, remoteURL string, in CheckoutInput) error {
	...
	case strings.HasPrefix(ref, "refs/tags/"):
		if err := git.Fetch(ctx, remoteURL, []string{"+" + ref + ":" + ref}, in.Token); err != nil {
	...
	case strings.HasPrefix(ref, "refs/heads/"):
		...
		if err := git.Fetch(ctx, remoteURL, []string{"+" + ref + ":" + dst}, in.Token); err != nil {
			return fetchError(ref, remoteURL, err)
		}
	...
	case isHexSHA(ref):
		if err := git.Fetch(ctx, remoteURL, []string{ref}, in.Token); err != nil {
			return fetchError(ref, remoteURL, err)
		}
...
func fetchBareName(ctx context.Context, git CheckoutGit, remoteURL string, in CheckoutInput) error {
	...
	if err := git.Fetch(ctx, remoteURL, []string{"+" + tagDst + ":" + tagDst}, in.Token); err == nil {
		return git.CheckoutDetach(ctx, tagDst)
	}

	branchDst := "refs/remotes/origin/" + ref
	if err := git.Fetch(ctx, remoteURL, []string{"+refs/heads/" + ref + ":" + branchDst}, in.Token); err == nil {
