...
// androidLibArtifacts mirrors examples/android-library: the artifact's
...
// forge-packages does not need signing or javadoc jars; only Central does.
...
`
	...
	for _, c := range checks {
		if strings.Contains(c.Name, "signing configured") || strings.Contains(c.Name, "sources + javadoc") {
			t.Errorf("central-only check ran for a forge-packages artifact: %q", c.Name)
		}
