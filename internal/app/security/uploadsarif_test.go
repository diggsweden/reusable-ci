...
func TestUploadSARIF_HappyPath_ForwardsPayloadToProvider(t *testing.T) {
	...
	err := appsecurity.UploadSARIF(context.Background(), prov, &stdout,
	...
	require.Contains(t, stdout.String(), "✓ SARIF accepted by Code Scanning")
}
...
func TestUploadSARIF_UnsupportedPlatformSkipsGracefully(t *testing.T) {
	t.Parallel()
	prov := fakeprovider.New(t).WithUploadSARIFError(errs.ErrUnsupported)
	var stderr bytes.Buffer
	err := appsecurity.UploadSARIF(context.Background(), prov, io.Discard,
		output.NewAnnotator(&stderr, output.FormatGitHub),
		appsecurity.UploadSARIFInput{
	...
	require.NoError(t, err)
	require.Contains(t, stderr.String(), "SARIF upload skipped — not supported on this platform")
}
...
func TestUploadSARIF_ProviderErrorPropagates(t *testing.T) {
	...
	require.Error(t, err)
	require.True(t, errors.Is(err, errs.ErrPermissionDenied))
