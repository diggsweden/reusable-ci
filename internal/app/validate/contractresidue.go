...
var defaultContractResidueRoots = []string{
	...
	".forgejo/workflows", // Forgejo Actions workflows (when present; tolerated if absent)
	"templates",          // GitLab CI/CD Catalog components
	"internal",
	"docs",
	"examples",
	"scripts",
...
var contractResidueRules = []contractResidueRule{
	...
	{
		name:    "legacy sbom generate all command",
		pattern: regexp.MustCompile(`\breusable-ci sbom generate all\b`),
		message: "renamed to `reusable-ci sbom assemble` (assembles the CISA layer set; --layers defaults to all)",
	},
	{
		name:    "legacy plan file-pattern command",
		pattern: regexp.MustCompile(`\breusable-ci plan file-pattern\b`),
		message: "moved to `reusable-ci version file-pattern` (it feeds the version-bump commit)",
	},
	{
		name:    "legacy version release-context command",
		pattern: regexp.MustCompile(`\breusable-ci version release-context\b`),
		message: "renamed to `reusable-ci version derive-release`",
	},
	{
		name:    "legacy build version-info command",
		pattern: regexp.MustCompile(`\breusable-ci build (?:gradle-android|xcode-ios) version-info\b`),
		message: "renamed to `... metadata` (unifies the build group's project-metadata verb)",
	},
	{
		name:    "legacy buildx/buildkit secret nomenclature",
		pattern: regexp.MustCompile(`buildx-secrets|buildkit-secrets`),
		message: "buildah replaced buildx: `materialize-build-secrets` emits the `secret-mounts` output into `$RUNNER_TEMP/build-secrets`",
	},
	{
		name:    "legacy expand-sboms --format flag",
		pattern: regexp.MustCompile(`\breusable-ci config expand-sboms\b.*--format\b`),
		message: "renamed to `--as json|comma` so it no longer shadows the global `--format` output renderer",
	},
	{
		name:    "legacy scan container --severity flag",
		pattern: regexp.MustCompile(`\breusable-ci security scan container\b.*--severity\b`),
		message: "renamed to `--fail-on-severity` to match the dependencies/opengrep scanners",
	},
	{
		name:    "legacy provenance --sign-method/--sign-key flags",
		pattern: regexp.MustCompile(`\breusable-ci release provenance\b.*--sign-(?:method|key)\b`),
		message: "renamed to `--method`/`--key` to match every other signing command",
	},
	{
		name:    "legacy artifacts-dir flag/env",
		pattern: regexp.MustCompile(`--artifacts-dir\b|\bARTIFACTS_DIR\b`),
		message: "renamed to `--artifact-dir` / $ARTIFACT_DIR (the canonical artifact-directory name)",
	},
	{
		name:    "legacy appstore prepare-credentials --out-dir flag",
		pattern: regexp.MustCompile(`\bprepare-credentials\b.*--out-dir\b`),
		message: "renamed to `--output-dir` (the canonical output-directory name)",
	},
	{
		name:    "legacy doctor --artifacts flag",
		pattern: regexp.MustCompile(`\breusable-ci doctor\b.*--artifacts\b(?:[^-]|$)`),
		message: "renamed to `--artifacts-file` (it is a single file, not a list; matches `config validate --file`)",
	},
	{
		name:    "legacy report status stage command",
		pattern: regexp.MustCompile(`\breusable-ci report status stage\b`),
		message: "moved to `reusable-ci report stage-result` (it composes a contract, not a `status` summary)",
	},
}
