...
func (f *fakeSBOMSigner) DetachSign(_ context.Context, _ string, file string) error {
	f.signed = append(f.signed, file)
	return os.WriteFile(file+".asc", []byte("sig"), 0o644)
}
...
func TestCreateSBOMZip_BundlesAllLayers(t *testing.T) {
	...
	r, err := zip.OpenReader(filepath.Join(dir, res.ZipName))
	if err != nil {
		t.Fatal(err)
	}
...
func TestCreateSBOMZip_VersionedZipName(t *testing.T) {
	tests := []struct {
		...
		{name: "keeps_prerelease_suffix", projectName: "myapp", version: "1.0.0-SNAPSHOT", fixture: "myapp-pom-sbom.spdx.json", want: "myapp-1.0.0-SNAPSHOT-sboms.zip"},
	}
...
func TestCreateSBOMZip_SigningSkippedWithoutFlagOrKey(t *testing.T) {
	fsys := testfs.NewReal(t)
	fsys.Chdir()
	dir := fsys.Root
	fsys.WriteFile("myapp-pom-sbom.spdx.json", []byte(`{}`))
	for _, testCase := range []struct {
		...
		{name: "flag false", in: apprelease.SBOMZipInput{ProjectName: "myapp", Version: "v1.2.3", SignArtifacts: false, GPGKeyID: "ABC123"}},
		{name: "key empty", in: apprelease.SBOMZipInput{ProjectName: "myapp", Version: "v1.2.3", SignArtifacts: true, GPGKeyID: ""}},
	} {
		t.Run(testCase.name, func(t *testing.T) {
			signer := &fakeSBOMSigner{}
