...
func TestReleaseFilesCollectManifestAndChecksumValidation(t *testing.T) {
	t.Chdir(t.TempDir())
	...
	for _, name := range []string{
		...
		if !releaseFileEntryNamed(assets.Assets, name) {
			t.Fatalf("expected release asset %q in %#v", name, releaseFileEntryNames(assets.Assets))
		}
	}
	...
	for _, name := range []string{
		...
		if releaseFileEntryNamed(assets.Assets, name) {
			t.Fatalf("unexpected release asset %q", name)
		}
	}
	...
	manifest, err := apprelease.WriteReleaseFileManifest(apprelease.WriteReleaseFileManifestInput{AssetsJSONFile: "assets.json", OutputFile: "dist/forgejo-ci-release-files.json"})
	if err != nil {
		t.Fatal(err)
	}
	...
	sboms, err := apprelease.ListReleaseFiles(apprelease.ListReleaseFilesInput{FilesInput: withManifest, Section: "sboms"})
	if err != nil {
		t.Fatal(err)
	}
