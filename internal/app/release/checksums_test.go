...
func TestChecksums_ReleaseArtifacts(t *testing.T) {
	...
	count, err := apprelease.Checksums(apprelease.ChecksumsInput{}, &bytes.Buffer{})
	if err != nil {
...
func TestChecksums_AttachArtifactsKeepsPath(t *testing.T) {
	...
	count, err := apprelease.Checksums(apprelease.ChecksumsInput{
...
func TestChecksums_CreatesEmptyOutputWhenNothingFound(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			count, err := apprelease.Checksums(testCase.input, &out)
			if err != nil {
				t.Fatalf("Checksums: %v", err)
			}
			if count != 0 {
...
func TestChecksums_CustomOutput(t *testing.T) {
	tests := []struct {
		...
		{name: "nested_path", outputFile: filepath.Join("output", "checksums.sha256"), wantPath: []string{"output", "checksums.sha256"}},
	}
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			fsys := testfs.NewReal(t)
			...
			count, err := apprelease.Checksums(apprelease.ChecksumsInput{OutputFile: testCase.outputFile}, &bytes.Buffer{})
			if err != nil {
				t.Fatal(err)
			}
			if count != 1 {
...
func TestChecksums_CommaSeparatedAttachPatterns(t *testing.T) {
	...
	count, err := apprelease.Checksums(apprelease.ChecksumsInput{AttachArtifacts: "file1.txt,file2.md"}, &bytes.Buffer{})
	if err != nil {
		t.Fatal(err)
	}
	if count != 2 {
	...
	for _, want := range []string{"file1.txt", "file2.md"} {
