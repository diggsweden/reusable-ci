...
func TestPromoteBaseImages_RefusesUnusableMetadata(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "ref carries a tag and digest", mutate: func(m *BaseImageMetadata) { m.Ref = promoteRepo + ":rust@" + promoteDigest }},
		...
		{name: "tag has a malformed repository path", mutate: func(m *BaseImageMetadata) { m.Tag = "codeberg.org/itiquette/BadName:rust" }},
...
func TestPromoteBaseImages_StopsWhenFinalLookupIsUncertain(t *testing.T) {
	t.Parallel()

	const source = "https://codeberg.org/itiquette/base"

	image := BaseImageMetadata{
		Flavor:       "rust",
		Tag:          promoteRepo + ":" + promoteInput + "-rust",
		Ref:          promoteRepo + "@" + promoteDigest,
		CandidateTag: promoteRepo + ":candidate-" + promoteInput + "-rust",
		CandidateRef: promoteRepo + "@" + promoteDigest,
		BaseInputID:  promoteInput,
	}

	registry := &fakeBaseImageRegistry{
		digests:    map[string]string{image.CandidateTag: promoteDigest},
		resolveErr: map[string]error{image.Tag: errs.ErrDependencyUnavailable},
	}
	verifier := &fakeBaseImageVerifier{payload: baseLineagePayload(t, lineageFields{
		source:      source,
		workflow:    ".forgejo/workflows/base-images.yml",
		flavor:      image.Flavor,
		baseInputID: image.BaseInputID,
	})}

	_, err := PromoteBaseImages(context.Background(), registry, verifier, io.Discard, BaseImagePromoteInput{
	...
	if len(registry.copied) != 0 {
		t.Fatalf("no base tag should be copied after an uncertain lookup, got %v", registry.copied)
	}
}
