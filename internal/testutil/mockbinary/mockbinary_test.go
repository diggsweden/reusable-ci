...
func TestMock_StubReturnsCannedOutput(t *testing.T) {
	...
	out, err := exec.Command(m.Path("gh"), "anything").Output()
	if err != nil {
...
func TestMock_RecordsInvocationsAndArgs(t *testing.T) {
	...
	for i := range 3 {
		if err := exec.Command("trivy", "image", "--severity", "HIGH", "alpine").Run(); err != nil {
...
func TestMock_AllNames(t *testing.T) {
	...
	_ = exec.Command("gh").Run()
	_ = exec.Command("trivy").Run()
