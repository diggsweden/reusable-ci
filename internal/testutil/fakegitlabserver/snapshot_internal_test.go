...
func TestServer_InMemorySnapshotOwnership(t *testing.T) {
	...
	want := []Request{
		{Method: http.MethodPost, Path: "/api/v4/projects/owner%2Frepo/releases", Body: []byte("first\n\x00"),
			Header: http.Header{"X-Fixture": {"one", "two"}}, Query: map[string][]string{"tag": {"first", "second"}, "blank": {""}}},
		{Method: http.MethodPut, Path: "/api/v4/projects/other/releases", Body: []byte("second\n"),
