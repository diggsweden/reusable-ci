...
// Package fakegitserver routes canned responses for GitHub adapter tests.
//
// Tests register canned responses per (METHOD, PATH) and hand the adapter
// Client(). The contract is identical to fakegitlabserver; the packages stay
// separate so each adapter test imports the right one and cannot cross-wire.
...
// GitHub adapter bound a loopback port and ran a listener goroutine. Nothing
...
func (s *Server) URL() string { return "https://github.invalid" }
...
func (s *Server) handle(w http.ResponseWriter, r *http.Request) { //nolint:varnamelen // idiomatic short name (testing/http/io conventions).
	...
	// Test-only GitHub API fake: responses are pre-canned fixture bytes, not
	// HTML or user-supplied data. html/template doesn't apply.
