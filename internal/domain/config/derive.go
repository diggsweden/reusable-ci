...
var (
	forgePackagesTypes = []projecttype.Type{
...
func SupportedPublishTarget(a Artifact, target PublishTarget) bool { //nolint:varnamelen // idiomatic short name (testing/http/io conventions).
	...
	case PublishForgePackages:
		if !projecttype.IsIn(a.ProjectType, forgePackagesTypes) {
			return false
		}

		// An Android artifact reaches forge packages only as a library —
		// an app's output is an APK/AAB, which is not a Maven artifact.
		//
		// Gradle *applications* are allowed on purpose: publishing an
		// application's jar to a forge registry is a legitimate internal
