...
func TestPublishTasks_Derived(t *testing.T) {
	...
	cases := []struct {
		...
		{publish.GradleTargetGitHubPackages, []string{"publishAllPublicationsToGitHubPackagesRepository"}},
...
func TestParseGradleTarget(t *testing.T) {
	...
	for _, raw := range []string{"github-packages", " maven-central "} {
		if _, err := publish.ParseGradleTarget(raw); err != nil {
			t.Errorf("ParseGradleTarget(%q) = %v, want ok", raw, err)
		}
	}

	for _, raw := range []string{"", "nexus", "GitHub-Packages", "maven_central"} {
...
func TestCredentialBindings(t *testing.T) {
	...
	gh := publish.CredentialBindings(publish.GradleTargetGitHubPackages)
	if len(gh) != 2 {
		t.Fatalf("github-packages bindings = %d, want 2", len(gh))
	}
...
func TestGradleRepositoryName(t *testing.T) {
	...
	if got := publish.GradleRepositoryName(publish.GradleTargetGitHubPackages); got != "GitHubPackages" {
		t.Errorf("got %q", got)
	}
