...
func NormalizeOpengrepFailSeverity(in string) (OpengrepSeverity, error) {
	...
	default:
		return "", fmt.Errorf("Unsupported OPENGREP_FAIL_ON_SEVERITY value: %s", in)
	}
}

// OpengrepHasFindingsMeetingThreshold mirrors the bash's
// has_findings_meeting_threshold semantics:
//
//   - none   → never fails
...
func RenderOpengrepSummary(in OpengrepSummaryInput) (markdown string, scanResult string) {
	scanResult = "success"
...
func RenderOpengrepFailureSummary(in OpengrepFailureSummaryInput) string {
	var b strings.Builder
	fmt.Fprintf(&b, "## OpenGrep SAST\n\n")
	fmt.Fprintf(&b, "OpenGrep exited with status %d before a complete result set was produced.\n\n", in.ExitCode)
	fmt.Fprintf(&b, "| Property | Value |\n")
	fmt.Fprintf(&b, "|----------|-------|\n")
	fmt.Fprintf(&b, "| Rules | %s |\n", in.Config)
	fmt.Fprintf(&b, "| Target | %s |\n", in.TargetPath)
	...
	fmt.Fprintf(&b, "| Workflow Run | %s |\n", OpengrepWorkflowRunLabel(in.Platform.RunURL))
	fmt.Fprintf(&b, "| Artifacts | %s |\n", OpengrepArtifactsLabel())
	fmt.Fprintf(&b, "| Scan Result | failure |\n")
	return b.String()
}
